...
func TestCreateEsIndexCleaner(t *testing.T) {
	...
	jaeger.Spec.Storage.EsIndexCleaner.AfterCompletionTTL = &afterCompletionTTL
	cronJob := CreateEsIndexCleaner(jaeger)
	assert.Equal(t, afterCompletionTTL, *cronJob.Spec.JobTemplate.Spec.TTLSecondsAfterFinished)
